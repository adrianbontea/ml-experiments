...
class ModelsExperiment(HousingExperimentBase): 
    async def do_run_async(self):
        ...
        print("Linear Regression RMSE for the whole trainig set is:", lin_rmse)

        # 68434 is not great at all! (basically means a typical prediction error of $68434). The model is underfitting (too simple)
        ...
        print("Decision Tree RMSE for the whole trainig set is:", lin_rmse)
        ...
        # Split the training set in 2, train using the first part and verify using the other
        ...
        # Finally, let's test the Random Forest (most promising from the 3 models although still overfitting quite a lot) on the test set!
        predictors = test_set.drop("median_house_value", axis = 1)
        labels = test_set.median_house_value.copy()

        predictors_tr = self.__transform(predictors)

        test_data = predictors_tr2[10:30:2]
        test_labels = labels2[10:30:2]
    ...
    def __transform(self, data):
        ...
        #predictors_tr is now a 2 dimensional nd array (16512,8) - Need to merge it with the binarized labels into (16512,13) shape
        # ocean proximity one hot vector is a 2 dimensional ndarray of shape (16512,5)
